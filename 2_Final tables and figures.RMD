...
pop_table <-
  list("Outcome measures" =
       list("Ever detect of PFOA" = ~n_perc(PFOA_detect, na_rm = T),
            ...
            "Ever detect of PFOA or PFOS" = ~n_perc(PFOA_PFOS_detect, na_rm = T),
       ...
       "Population served [millions]" =
        list("Ever detect of PFOA" = ~round(sum(ifelse(PFOA_detect==1,
             ...
             "Ever detect of PFBS" = ~round(sum(ifelse(PFBS_detect==1,
                                            population_served, 0), na.rm = T)/10^6, 1),
             "Ever detect of PFOA or PFOS" = ~round(sum(ifelse(PFOA_PFOS_detect==1,
                                            population_served, 0), na.rm = T)/10^6, 1),
...
CWS_FN$PFOA_PFOS_detect <- ifelse(CWS_FN$PFOA_detect > 0 | CWS_FN$PFOS_detect > 0 &
                                  !is.na(CWS_FN$PFOA_detect) & !is.na(CWS_FN$PFOS_detect),
                                   1, 0)
...
SDWIS <- SDWIS %>% dplyr::select(WATER_SYSTEM.PWSID, WATER_SYSTEM.PWS_NAME,
  ...
  dplyr::rename(PWSID = WATER_SYSTEM.PWSID,
                ...
                Activity_code = WATER_SYSTEM.PWS_ACTIVITY_CODE,
...
# SDWIS with all states that are active as of this SDWIS pull
SDWIS <- SDWIS %>%
...
SDWIS <- SDWIS %>% filter(PWS_type == "CWS" & Activity_code == "A")
  ...
  list("Table 1" =
       list(
         "Number of large or very large systems" = ~n_perc(PWS_size_tri == "Large or very large", na_rm=T, digits = 1),
            ...
            "Number of small or very small systems" = ~n_perc(PWS_size_tri == "Small or very small", na_rm=T, digits = 1),
            "Number of systems with SW" = ~n_perc(GW_SW == "SW", na_rm = T, digits = 1),
            "Total population served [millions]" = ~round(sum(population_served)/1000000, 1))
       )
...
# HUC data: Spatial regression models (figure for presentations only)
...
# wide version
wide_main.models <- HUC_main.models %>% select(outcome.name, dep.var, full.coefficient, n.obs, R2)
wide_main.models <- pivot_wider(wide_main.models, 
                                id_cols = c("outcome.name", "n.obs", "R2"),
                                names_from = dep.var, 
                                values_from = full.coefficient)

knitr::kable(wide_main.models %>% select(-"(Intercept)"))
...
wide_exclude.NDs <- HUC_exclude.NDs %>% select(outcome.name, dep.var, full.coefficient, n.obs, R2)
wide_exclude.NDs <- pivot_wider(wide_exclude.NDs, 
                                id_cols = c("outcome.name", "n.obs", "R2"),
                                names_from = dep.var, 
                                values_from = full.coefficient)

knitr::kable(wide_exclude.NDs %>% select(-"(Intercept)"))
...
# HUC maps (for presentations only)
```{r}
# categories for map
HUC.PFAS <- HUC.PFAS %>%
...
# filtering out HUCs containing great lakes for clarity
...
# Additional HUC maps (for presentations only)
```{r}
...
ggplot(all.sources_models, aes(x = OR, y = dep.var.name2, 
    ...
    labs(x = "Odds ratio", y = "",
         subtitle = "Odds ratios [95% CI] corresponding to one-percentage-point higher proportions of\neach sociodemographic factor") +
    theme_bw() +
...
if (FALSE) {
  ggsave("../Figures/sources_ORs.png", dpi = 600, height = 8, width = 10)
}
...
source("1d_Secondary + sensitivity analysis modelling.R")
...
```{r}
# this is loaded from the statewide sampling repository
DLs.table <- read.csv("/Users/jahredl/Dropbox/State PFAS data/Merged PFAS data/DL_table.csv")
...
# how many samples are from the FINAL set of CWS? (after apply all exclusion criteria)
nrow(dat[dat$PWSID %in% CWS_FN$PWSID, ])

DLs <- DLs %>% filter(Compound != "PFBA_DL" & Compound != "HFPODA_DL")
...
CWS_FN$missing <- is.na(CWS_FN$Any_detect)

missing.Any <- plot_pattern(CWS_FN %>% select(PFOA = PFOA_detect, PFOS = PFOS_detect,
                                              PFNA = PFNA_detect, PFHxS = PFHxS_detect,
                                              PFBS = PFBS_detect, "All" = Any_detect))

missing.Any +
  scale_fill_manual(values = c("red", "blue"), labels = c("Missing", "Observed")) +
  scale_alpha(range = 0.8, guide = "none") +
  theme_mice() +
  theme(axis.title.x.top = element_blank(),
        axis.text.x.top = element_text(size = 15, face = "bold"),
        axis.text = element_text(size = 15),
        axis.title = element_text(size = 15, margin = margin(l = 0.25, r = 0.25)),
        legend.text = element_text(size = 12))

ggsave("../Figures/missingness.png", dpi = 600, height = 6, width = 9)
